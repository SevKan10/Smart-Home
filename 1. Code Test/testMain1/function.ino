...
{
  static bool staMenu = 1;
  if (digitalRead(MENU) == 0 && staMenu == 1) {Ring(1,50); flagMenu=1; flagMode=1;}
  staMenu = digitalRead(MENU);

  static bool staSelectTime = 1;
  if (digitalRead(SEL) == 0 && staSelectTime == 1) {
    Ring(1, 50);
    flagSelect++;
  }
    if (flagSelect > 2) {flagSelect = 1;}
    switch (flagSelect) {
      case 1:
        lcd.setCursor(0, 0);
        lcd.print("<Back");
